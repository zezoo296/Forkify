...
class RecipeView extends View{
    ...
    _errorMsg = "Couldn't find that recipe please try again!";
    _successMsg;
    ...
    _generateMarkup(){
      ...
      </div>` : `<div class="recipe__user-generated">
      <svg>
        <use href=""></use>
      </svg>
    </div>`}
    ...
    addHandlerRender(handler){
      ...
      events.forEach(e => window.addEventListener(e, () => handler(window.location.hash.slice(1))));
    }
    ...
    addHandlerUpdateServings(handler){
      this._parentElement.addEventListener('click', function(e){
        if(e.target.closest('.btn--increase-servings'))
          handler(1);
        else if (e.target.closest('.btn--decrease-servings'))
          handler(-1);
        else
          return;
      })
